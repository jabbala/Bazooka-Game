...
void Hero::init(std::string textureName, int fireCount, float animDuration, sf::Vector2f position, float mass)
{
	m_position = position;
	m_mass = mass;
	m_grounded = false;
...
void Hero::update(float dt)
{
	//Animation
	m_elapsedTime += dt;
	int animFrame = static_cast<int>((m_elapsedTime / m_animDuration) * m_fireCount) % m_fireCount;
